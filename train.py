...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = ''  # 加载的预训练权重一定要和backbone对应
    #------------------------------------------------------#
    ...
    #------------------------------------------------------#
    train_annotation_path   = '2007_train.txt' # 用于训练
    val_annotation_path     = '2007_val.txt' # 用于测试而非验证
    ...
    #   创建yolo模型
    #------------------------------------------------------#
    '''pretrained用于判断是否加载主干的权重，具体看源码，这一点非常重要，因为如果是修改了neck结构，比如加了注意力机制，
    那么整个网络的权重是不能用了，但是可以加载主干的权重，此时只能从主干开始训练'''
    model = YoloBody(anchors_mask, num_classes)
    if pretrained:
        weights_init(model)
        # model_dict = model.state_dict()
        model_dict = model.backbone.state_dict()
        backbone_weights_path = 'nets/efficientnet-b0-355c32eb.pth'
        pretrained_dict = torch.load(backbone_weights_path)
    ...
    if local_rank == 0:
        ...
        #   总训练步长指的是梯度下降的总次数 （interation * epoch），对一个batch的数据进行一次梯度下降
